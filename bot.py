...
try:
    ...
    raise RuntimeError("ADMIN_CHAT_ID должен быть числом (chat_id администратора)")

# URL твоего сервиса на Railway
APP_URL = os.getenv("APP_URL", "https://skvmestebot-production.up.railway.app").rstrip("/")
...
async def about(message: types.Message):
    text = (
        "Строительная компания СК «Вместе» — это команда архитекторов, инженеров и специалистов, которые создают "
        "надёжные дома, продуманные проекты и комфортные пространства для жизни. Мы работаем «под ключ» и берём "
        "на себя всё: от идеи и проектирования до строительства, инженерии, чистовой отделки и благоустройства "
        "территории.\n\n"
        "Наш принцип прост — делаем так, как сделали бы для себя. Каждый проект — это не просто квадратные метры, "
        ...
        "Мы работаем открыто и честно: фиксированная смета, прозрачные процессы, регулярные отчёты, фото-видео с "
        "объектов. С нами клиенты понимают, за что платят, и получают именно тот результат, который ожидают.\n\n"
        ...
        "Если вы хотите построить дом, заказать архитектурный проект или подобрать готовое решение — оставьте ваш "
        "номер телефона или напишите нам прямо сейчас.\n"
...
async def build_q1(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_reply_markup(
        build_keyboard(["Кирпич", "Монолит", "Газобетон", "Пока не определился"])
    )
...
async def build_q2(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_reply_markup(
        build_keyboard(["До 100 м²", "100-150 м²", "150-200 м²", "Больше 200 м²"])
    )
...
async def build_q5(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "📲 Оставьте номер телефона для связи:", reply_markup=phone_kb()
    )
...
async def build_phone_contact(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "✅ Спасибо! Наш менеджер скоро свяжется с вами.", reply_markup=main_menu()
    )
    await state.finish()
...
async def project_q5(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(q5=call.data)
    await call.message.answer(
        "📲 Оставьте телефон для связи с архитектором:", reply_markup=phone_kb()
    )
...
async def form_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "📱 Теперь отправьте номер телефона:", reply_markup=phone_kb()
    )
...
async def on_shutdown(dp: Dispatcher):
    logger.info("Удаляю webhook и закрываю хранилище")
