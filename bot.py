...
async def quiz_build_start(message: types.Message, state: FSMContext):
    await state.finish()
    await state.update_data(quiz="build")                     # помечаем, что это квиз строительства

    # Фото + текст (оставь свои)
    ...
    await message.answer("Чтобы рассчитать ориентировочную стоимость дома, ответьте на несколько вопросов.\n"
...
# Вопрос 2
@dp.callback_query_handler(lambda c: c.data.startswith("qb1_"))
...
# Вопрос 3
@dp.callback_query_handler(lambda c: c.data.startswith("qb2_"))
async def qb_q3(call: types.CallbackQuery, state: FSMContext):
    await call.answer()
    answers = {"qb2_1": "Кирпич", "qb2_2": "Каркас / Брус", "qb2_3": "Газобетон / Монолит", "qb2_4": "Пока не определился, нужна консультация"}
...
# Вопрос 4
@dp.callback_query_handler(lambda c: c.data.startswith("qb3_"))
async def qb_q4(call: types.CallbackQuery, state: FSMContext):
    await call.answer()
    answers = {"qb3_1": "до 100 м²", "qb3_2": "100–150 м²", "qb3_3": "150–200 м²", "qb3_4": "Больше 200 м²"}
...
# Вопрос 5
@dp.callback_query_handler(lambda c: c.data.startswith("qb4_"))
async def qb_q5(call: types.CallbackQuery, state: FSMContext):
    await call.answer()
    answers = {"qb4_1": "Есть готовый проект",
...
# Телефон после последнего вопроса
@dp.callback_query_handler(lambda c: c.data.startswith("qb5_"))
async def qb_phone(call: types.CallbackQuery, state: FSMContext):
    await call.answer()
    answers = {"qb5_1": "В ближайшее время", "qb5_2": "Через 1–3 месяца", "qb5_3": "Через 3–6 месяцев", "qb5_4": "Не знаю, нужна консультация"}
...
async def quiz_project_start(message: types.Message, state: FSMContext):
    ...
    full_text = (
        ...
        "<b>Рассчитаем смету будущего строительства!</b>"
